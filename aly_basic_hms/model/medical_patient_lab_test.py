...
class medical_patient_lab_test(models.Model):
    ...
    def create(self, vals):
        vals['request'] = self.env['ir.sequence'].next_by_code('medical.patient_test_seq')
    ...
    def create_lab_test(self):
        ...
        for browse_record in self:
            ...
            res=medical_lab_obj.create({
                                        'name': self.env['ir.sequence'].next_by_code('medical.lab_test_seq'),
            ...
            if res_ids:                     
                ...
                action = imd.xmlid_to_object('action_medical_lab_form')
                list_view_id = imd.xmlid_to_res_id('medical_lab_tree_view')
                form_view_id  =  imd.xmlid_to_res_id('medical_lab_form_view')
                result = {
                                'name': action.name,
                                'help': action.help,
                                'type': action.type,
