...
class medical_lab_test_create(models.TransientModel):
    ...
    def create_lab_test(self):
        ...
        for browse_record in browse_records:
            ...
            res=medical_lab_obj.create({'name': self.env['ir.sequence'].next_by_code('medical.lab_test_seq'),
            ...
            if res_ids:
                ...
                action = imd.sudo().xmlid_to_object('aly_basic_hms.action_medical_lab_tree')
                list_view_id = imd.sudo().xmlid_to_res_id('aly_basic_hms.medical_lab_tree_view')
                form_view_id  =  imd.sudo().xmlid_to_res_id('aly_basic_hms.medical_lab_form_view')
                result = {
                                'name': action.name,
                                'help': action.help,
                                'type': action.type,
