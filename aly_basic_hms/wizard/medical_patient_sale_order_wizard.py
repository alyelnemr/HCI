...
class MedicalPatientSaleOrderWizard(models.TransientModel):
    ...
    def create_invoice(self):
        ...
        for active_id in active_ids:
            ...
            price_list_id = self.env.user.pricelist_id.id if self.env.user.pricelist_id else medical_patient_obj.insurance_company_id.property_product_pricelist.id if medical_patient_obj.is_insurance else medical_patient_obj.partner_id.property_product_pricelist.id or False
            ...
            if not medical_patient_obj.invoice_id.state == 'posted':
                ...
                all_old_inv = sale_order_obj.search([('partner_id', '=', partner_id)])
                all_old_inv.invoice_ids.write({
                    'state': 'cancel'
                })
                all_old_inv.write({
                    'state': 'cancel'
                ...
                sale_order_id = sale_order_obj.create(invoice_vals)

                sale_order_lines_list = []
                ...
                if medical_patient_obj.company_id.aly_enable_service_charge and medical_patient_obj.clinic_id.is_hospital:
                    sale_order_lines_list.append({
                        ...
                        'product_uom_qty': 1,
                        'price_unit': 0,
                        'product_id': medical_patient_obj.company_id.aly_service_product_id.id,
                        'order_id': sale_order_id.id
                    })
                    # res1 = sale_order_line_obj.create(invoice_line_vals)

                # res1 = account_invoice_line_obj.create({'order_line': list_of_vals})
                sale_order_line_obj.create(sale_order_lines_list)

                sale_order_id.update_prices()
                medical_patient_obj.order_id = sale_order_id
                medical_patient_obj.order_id = sale_order_id
                medical_patient_obj.with_context({'come_from_invoice': True}).is_opened_visit = False

                list_of_ids.append(sale_order_id.id)
                if list_of_ids:
                    ...
                    result = {
                        ...
                        'params': {
                                'title': _(
                                    "Invoice Created Successfully for Patient [%s] with Invoice Number [%s]" % (medical_patient_obj.name, sale_order_id.name)
                                ),
                                'type': 'success',
                                'sticky': True,
